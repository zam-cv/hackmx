...
impl<'a> DocService<models::Document, UploadForm> for DocumentsService<'a> {
    ...
    async fn db_create_document(&self, document: models::Document) -> anyhow::Result<i32> {
        self.0.create_document(document).await
    }

    async fn db_delete_document_by_id(&self, id: i32) -> anyhow::Result<models::Document> {
        self.0.delete_document_by_id(id).await
    }

    async fn db_get_document_by_id(&self, id: i32) -> anyhow::Result<Option<models::Document>> {
        self.0.get_document_by_id(id).await
    }
}
...
pub async fn get_documents(
    ...
    match database
        .get_documents_by_event_id(event_id.into_inner())
        .await
    {
        Ok(mut documents) => {
            let service = DocumentsService(&database);
